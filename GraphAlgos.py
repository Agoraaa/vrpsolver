...
class GraphAlgos:
    def dsu_find_parent(node, parents):
        if parents[node] == node:
            return node
        return GraphAlgos.dsu_find_parent(parents[node], parents)

    def dsu_merge_sets(node1, node2, parents, counts):
        node1 = GraphAlgos.dsu_find_parent(node1, parents)
        node2 = GraphAlgos.dsu_find_parent(node2, parents)
        if node1 == node2:
            return
    ...
    def minimum_spanning_tree(dist_mat, initial_edges = None):
        ...
        for (edge_start, edge_end, cost) in edge_list:
            if GraphAlgos.dsu_find_parent(edge_start, parents) == GraphAlgos.dsu_find_parent(edge_end, parents):
                continue
            ...
            GraphAlgos.dsu_merge_sets(edge_start, edge_end, parents, counts)
            res.append((edge_start, edge_end))
            if counts[GraphAlgos.dsu_find_parent(edge_start, parents)] == len(dist_mat):
